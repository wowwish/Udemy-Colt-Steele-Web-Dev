...
// EXAMPLE OF ONE-TO-MANY RELATIONSHIP USING TWO SEPERATE DEFINED MODELS


//  Defining the two Schemas
...
/* const makeFarm = async () => {
    ...
    console.log(farm); // The output will look like we pushed an entire object into the 'products' array of
    // 'farm', but if you save it and check it from Mongo, using db.farms.find({}), you will see that 'products' 
    // property of the farm only has an ObjectID like this:
    // "products" : [ ObjectId("6280a56c0bc23099d1039c07") ] 
    await farm.save();
}
 */
// makeFarm();

// One-time-function to populate a document from the Product model into a document of the Farm model through reference
...
// This prints only the IDs for 'products' property
Farm.findOne({ name: 'Full Belly Farms' })
  .then(farm => console.log(farm));

// This will print the 
Farm.findOne({ name: 'Full Belly Farms' })
  .populate('products') // Used to replace the ObjectID values of the 'products' property array with the 
  // corresponding mongoose documents returned from the database by performing a separate query before returning 
  // the results.
  .then(farm => console.log(farm));
